...
/** Recognizes lexemes for the mini-Pascal language by implementation of a 
 * state machine. Makes each lexeme available one at a time. It uses a lookup 
 * table to assign each lexeme a token type. 
			...
			switch(stateNumber) {
				case START:
					if(currentCharacter == -1) {
						/* Has reached the end of the file, so it sets both 
						 lexeme and token to null */
						this.lexeme = null;
						this.type = null;
						return false;	
					}
